...
public class fordfulkerson {
	private static final String PROG = "fordfulkerson";
	...
	private static void init() {
		...
		for (int edge = 0; edge < m; edge++)
			edges[scanner.nextInt()][scanner.nextInt()] =
					new Arc(scanner.nextInt(), scanner.nextInt());
	}
	...
	private static void output() {
		int result = 0;
		for (int u = 0; u < n; u++) for (int v = 0; v < n; v++)
			if (edges[u][v].capacity > 0) {
				if (u == 0) result += edges[u][v].flow;
				out.print(u);
				out.print(' ');
				out.print(v);
				out.print(' ');
				out.println(edges[u][v].flow);
			}
		out.println(result);
	}
